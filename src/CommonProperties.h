...
///
/// \sa CheckBox, CommandButton, Frame, OptionButton, StringProperties
//////////////////////////////////////////////////////////////////////
...
public:
	...
	CommonProperties();
		...
		BEGIN_MSG_MAP(CommonProperties)
			...
			COMMAND_ID_HANDLER(IDC_USEIMPROVEDIMGLSTSUPPORTCHECK, OnUseImprovedImageListSupport)
			COMMAND_ID_HANDLER(IDC_SETICONINDEXBUTTON, OnSetIconIndex)
			COMMAND_ID_HANDLER(ID_LOADSETTINGS, OnLoadSettingsFromFile)
			COMMAND_ID_HANDLER(ID_SAVESETTINGS, OnSaveSettingsToFile)

			COMMAND_HANDLER(IDC_ICONINDEXESCOMBO, CBN_SELCHANGE, OnIconIndexSelChange)
			COMMAND_HANDLER(IDC_ICONINDEXEDIT, EN_CHANGE, OnIconIndexChange)
...
protected:
	...
	///
	//@{
	/// \brief <em>\c IDC_USEIMPROVEDIMGLSTSUPPORTCHECK handler</em>
	///
	/// Will be called if the \c useImprovedImgLstSupportCheck control was clicked.
	/// We use this handler to enable or disable other controls.
	LRESULT OnUseImprovedImageListSupport(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/);
	/// \brief <em>\c IDC_SETICONINDEXBUTTON handler</em>
	///
	/// Will be called if the \c IDC_SETICONINDEXBUTTON command was initiated.
	/// We use this handler to execute the command.
	LRESULT OnSetIconIndex(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/);
	...
	///
	/// Will be called if the icon indexes combo box control's selection was changed.
	/// We use this handler to update the value displayed in the icon index edit control.
	LRESULT OnIconIndexSelChange(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/);
	/// \brief <em>\c EN_CHANGE handler</em>
	///
	/// Will be called if the icon index edit control's content was changed.
	/// We use this handler to set the dirty flag.
	LRESULT OnIconIndexChange(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/);
	//@}
	//////////////////////////////////////////////////////////////////////
	...
	struct Controls
	{
		...
		/// \brief <em>Wraps the check box control displaying the values for the \c UseImprovedImageListSupport property</em>
		CButton useImprovedImgLstSupportCheck;
		/// \brief <em>Wraps the combo box control displaying the indexes for the \c IconIndex property</em>
		CComboBox iconIndexesCombo;
		/// \brief <em>Wraps the edit control used to edit the indexes for the \c IconIndex property</em>
		CEdit iconIndexEdit;
		/// \brief <em>Wraps the button control used to edit the indexes for the \c IconIndex property</em>
		CButton setIconIndexButton;
	...
	struct Properties
	{
		...
		/// \brief <em>The buffered value of the \c IconIndex properties</em>
		LONG iconIndexes[6];
